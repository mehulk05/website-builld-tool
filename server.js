...
async function runEnrichJob(job) {
  ...
  try {
    ...
    const resolved = resolveBrand(themeAbs, composed, fromBuild);
    composed = resolved.brand;
    const themeBrand = resolved.themeBrand || readThemeBrand(themeAbs);
    const after = `${composed.headingFont}/${composed.bodyFont} ${composed.primary}`;
    if (before !== after) log_srv(`brand from ${resolved.from}: ${before} -> ${after}`);
    job.brandSource = {
      from: resolved.from,
      theme: P.themeSlug,
