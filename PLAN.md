...
| H6 | Attached pictures die on every deploy | ⏳ pending | render replaces the tool's disk on every release. Interim that scales: point upload.js at a persistent disk via an env var — it is a hardcoded path today. Real fix is H9 |
...
| H9 | Media through git | 🚫 blocked | six probes: import works, the sidecar shape is right, and the URL is predictable (uploads/YYYY/MM/<filename>, name kept, no -scaled). An unreferenced file is skipped, and the only anchor that works is featured_image on a published page — which we will not use, because it writes a review photo into a real content field on every client site. Ask is one line: make deploy import resources/media even when unreferenced. INFRA-media-on-deploy.md |
