...
| H6 | Attached pictures die on every deploy | ⚠ partial | upload.js now reads FEEDBACK_UPLOAD_DIR, so pointing uploads at storage that survives a release is a host setting rather than a release. Turning it on is a decision about the render plan, and it is a stopgap either way — the picture still depends on this tool being reachable. H9 is the real fix |
...
| H9 | Media through git | 🚫 blocked | nine probes. Import works and the URL is predictable; an unreferenced file is skipped, and the ONLY anchor is featured_image on a real content resource — page, post, astra-portfolio. Not a draft, not an Elementor image field, not a cpt.json. So every workaround costs a published content resource per photo, and a page has one featured image. Verified against a working example in G99agency/ehdenmedical.com PR #2. Ask is in INFRA-media-on-deploy.md |
