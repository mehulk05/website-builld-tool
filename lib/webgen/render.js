...
const LUXE = `
  ...
  /* 4 — restrained colour: headings stay ink, accent only on hairlines / labels / stats */
  h1,h2,h3,.card h3,.svc-row h3,.quote h4{color:var(--ink)}
  .testi h2,.cats h2{color:#fff}
`;
...
function renderFromBlueprint(kit, bp, title, pool) {
  ...
  for (const b of arr(bp.blocks)) {
    ...
    else if (b.type === "cards") for (const c of arr(b.cards)) c.image = pick(c.image);
