...
function header(k, d) {
  ...
  <header><nav class="nav wrap">
      <ul id="menu"><li><a href="index.html">Home</a></li><li><a href="services.html">Services</a></li><li><a href="about.html">About</a></li><li><a href="contact.html">Contact</a></li></ul>
      <a class="logo" href="index.html">${esc(b.name || "MED SPA")}${d.logoSpan ? `<span>${esc(b.sub || "")}</span>` : ""}</a>
      <div class="nav-right"><a class="phone" href="tel:${esc(b.phone || "")}">${esc(b.phone || "")}</a><a class="btn" href="contact.html">${esc(d.book)}</a>
...
function heroSection(k, d) {
  ...
  if (d.hero === "split") {
    ...
    return `<section class="hero hero--split"><div class="wrap hero-grid">
        <div class="hero-copy"><p class="eyebrow">${esc(h.eyebrow)}</p><h1>${esc(h.h1)}</h1>
          ...
          <div class="hero-actions"><a class="btn" href="contact.html">${esc(h.cta || "Book a Visit")}</a>
            <a class="hero-alt" href="services.html">Explore treatments</a></div></div>
  ...
  if (d.hero === "centered") {
    return `<section class="hero"><div class="wrap"><p class="eyebrow">${esc(h.eyebrow)}</p><h1>${esc(h.h1)}</h1><p>${esc(h.body)}</p>
        <a class="btn" href="contact.html">${esc(h.cta || "Book a Visit")}</a></div>${img ? `<div class="hero-media">${img}</div>` : ""}</section>`;
  }
  ...
  return `<section class="hero"><div class="hero-bg">${img}</div><div class="wrap">
      ...
      <a class="btn btn--light" href="contact.html">${esc(h.cta || "Book a Visit")}</a></div></section>`;
}
...
function aboutSection(k, d) {
  ...
  return `<section class="${cls}" id="about"><div class="wrap about-grid${hasImg ? "" : " solo"}">
      <div class="about-copy"><p class="eyebrow">${esc(a.eyebrow)}</p><h2>${esc(a.h2)}</h2>
        <div style="margin-top:24px">${paras}</div><a class="btn" href="contact.html">${esc(a.cta || "Book Now")}</a></div>
...
function specialtiesSection(k, d) {
  ...
  const cards = arr(sp.cards).map((c, i) => `
      <div class="card${c.image ? "" : " card--text"}">${d.cardIdx ? `<span class="idx">0${i + 1}</span>` : ""}${c.image ? `<div class="card-img">${cover(c.image)}</div>` : ""}<div class="card-body">
        <h3>${esc(c.h3)}</h3><p>${esc(c.p)}</p><a class="btn" href="services.html">Learn More</a></div></div>`).join("");
...
function featuredSection(k, d) {
  ...
  const html = items.map((x) => `<article><h3>${esc(x.h3)}</h3><p>${esc(x.p)}</p><a class="btn" href="contact.html">Book Now</a></article>`).join("");
...
function servicesMenuSection(k, d) {
  ...
  const rows = m.map((x, i) => `<article class="svc-row"><span class="svc-n">${String(i + 1).padStart(2, "0")}</span><div><h3>${esc(x.name)}</h3><p>${esc(x.desc)}</p></div><a class="btn" href="contact.html">Book</a></article>`).join("");
...
function blockCards(b, d) {
  ...
  const html = cs.map((c) => `<div class="card">${c.image ? `<div class="card-img">${cover(c.image)}</div>` : ""}<div class="card-body"><h3>${esc(c.h3)}</h3><p>${esc(c.p)}</p><a class="btn" href="contact.html">Learn More</a></div></div>`).join("");
...
function blockList(b, d) {
  ...
  const rows = it.map((x, i) => `<article class="svc-row"><span class="svc-n">${String(i + 1).padStart(2, "0")}</span><div><h3>${esc(x.name)}</h3><p>${esc(x.desc)}</p></div><a class="btn" href="contact.html">Book</a></article>`).join("");
