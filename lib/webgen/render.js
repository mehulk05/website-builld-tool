...
document.querySelectorAll('section,footer').forEach(function(sec){var kids=sec.querySelectorAll('.card,.member,.quote,.feat article,.about-copy,.about-img,.center,.gcell,.hero .wrap>*');var g=kids.length?kids:[sec];g.forEach(function(el,i){if(rm)return;el.setAttribute('data-reveal','');el.style.transitionDelay=(Math.min(i,8)*80)+'ms';io.observe(el);});});
