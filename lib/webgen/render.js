...
// SEMANTIC ANCHOR. A designer's change has to point at something that still
// exists after the site is regenerated, and Elementor's own container ids cannot
// do that: they are md5(slug|sec|index), so they are positional — inserting one
// section renames every later one, and because only the index feeds the hash the
// same ids appear on every client site in the fleet.
//
// This renderer is code, so at render time it knows what each element IS. Stamp
// that instead: role-based, not position-based. `hero.h1` still means the hero
// headline after a section is inserted above it, and means the same thing on
// every site. Anchors are also what let a picker resolve a click to a stable
// address, and what the Elementor compiler can use for container ids.
//
// Verified safe to ship: data-* attributes survive the WordPress import and are
// present on the live client site.
//
// A path must be identical across every design variant of the same element —
// heroSection has three branches and all three stamp hero.h1 — otherwise the
// anchor means different things depending on which layout the AI picked.
const A = (path) => (path ? ` data-g99="${path}"` : "");

// An anchor has to survive BOTH compile paths, and they keep different things:
//
//  * html widget   — ships markup verbatim, so `data-g99` survives as-is.
//  * native widget — lib/gitops/widgets.js turns headings, buttons, images and
//                    body copy into real Elementor widgets whose settings hold
//                    `title`/`text`/`url`, NOT the original attributes. A
//                    `data-*` there is dropped. But that converter already
//                    copies the node's CSS classes into `settings._css_classes`.
//
// So mirror every anchor into a class as well. It rides through native
// conversion for free, renders as a real class on the page, and is what
// overrides.css targets — one uniform selector regardless of widget type.
// Dots become `__` so the result needs no CSS escaping: hero.h1 → g99--hero__h1.
const anchorClass = (path) => "g99--" + String(path).replace(/\./g, "__");

// Applied once to a finished document. A post-pass rather than 40 hand-merges
// into existing class attributes — the markup is ours and data-g99 is unique per
// tag, so this is a safe rewrite, and it is covered by test-webgen.js.
function mirrorAnchorsToClasses(html) {
  return String(html).replace(/<([a-zA-Z][\w-]*)((?:"[^"]*"|'[^']*'|[^>"'])*)>/g, (m, tag, attrs) => {
    const hit = /\sdata-g99="([^"]+)"/.exec(attrs);
    if (!hit) return m;
    const cls = anchorClass(hit[1]);
    if (new RegExp(`\\b${cls}\\b`).test(attrs)) return m;          // already mirrored
    // APPENDED, never prepended. The design's own class has to stay first:
    // widgets.js keys grid handling off the leading class (OPAQUE_DIV), and
    // test-webgen.js asserts `class="card…"` because turning a card grid into
    // containers was tried and broke the live layout.
    const merged = /\sclass="/.test(attrs)
      ? attrs.replace(/(\sclass=")([^"]*)(")/, `$1$2 ${cls}$3`)
      : attrs.replace(/(\s*\/?)$/, ` class="${cls}"$1`);
    return `<${tag}${merged}>`;
  });
}

const cover = (url, anchor) => (url ? `<img${A(anchor)} src="${esc(url)}" alt="" loading="lazy" onerror="var p=this.closest('.card-img,.photo,.about-img,.hero-media,.page-hero-bg,.hero-bg');if(p){p.remove();}else{this.remove();}" />` : "");
...
const SHARED = `
  *{box-sizing:border-box;margin:0;padding:0}html{scroll-behavior:smooth}
  ...
  .foot-logo{max-height:54px;width:auto;margin-bottom:20px;filter:brightness(0) invert(1);opacity:.92}
  .foot-brand>p{max-width:34ch;opacity:.95;line-height:1.7}
...
const LUXE = `
  ...
  /* --card-ink lets a dark design opt out: this layer assumes light cards, but
     bold paints .card with --ink, which made these headings ink-on-ink (1.00:1). */
  .card h3,.svc-row h3,.value h3{color:var(--card-ink,var(--ink))}
  ...
  .cta .btn{background:var(--accent);border-color:var(--accent);color:var(--ink)}
  ...
  .hero--split .btn:hover{background:var(--accent);border-color:var(--accent);color:#fff}
...
const CSS_BOLD = `
  ...
  /* bold paints the page with --ink and whitens every .btn to suit, but .about is
     its one LIGHT band — a white button on cream measured 1.36:1, i.e. invisible.
     Headings are already pulled back to ink just above; the button needs the same. */
  .about .btn{border-color:var(--ink);color:var(--ink)}
  .about .btn:hover{background:var(--ink);color:var(--cream)}
  ...
  .card{background:var(--ink);--card-ink:#fff;position:relative;overflow:hidden}.card-img{aspect-ratio:16/11;overflow:hidden}.card-img img{width:100%;height:100%;object-fit:cover;transition:transform .6s}.card:hover .card-img img{transform:scale(1.07)}
...
const CSS_LUXE = `
  ...
  /* luxe is configured strip:"marquee", so stripSection emits .track — not .wrap.
     Only CSS_BOLD ever defined a .track rule, so on this design the marquee had no
     flex, no nowrap and no animation: the spans wrapped onto a second line and the
     deliberate duplication (needed for a seamless loop) just read as repeated text. */
  .strip .track{display:flex;width:max-content;gap:56px;white-space:nowrap;font-family:var(--serif);font-style:italic;font-size:19px;letter-spacing:.04em;animation:marq 34s linear infinite}
  .strip:hover .track{animation-play-state:paused}
  @keyframes marq{to{transform:translateX(-50%)}}
  @media (prefers-reduced-motion: reduce){.strip .track{animation:none;flex-wrap:wrap;width:auto;justify-content:center;white-space:normal}}
...
// Tone-driven surface treatment. These used to be chosen by hashing the business
// name — so a client who told us "understated" on a slider could still be handed
// pill buttons and 20px rounded cards. The sliders decide now; the hash survives
// only as the fallback for a kit carrying no tone at all.
const DENSITY_PAD = { tight: 0.9, normal: 1, airy: 1.16, expansive: 1.34 };
const RADIUS_PX = { none: "0", sm: "4px", md: "14px", pill: "22px" };
const BTN_RADIUS = { none: "0", sm: "0", md: "999px", pill: "999px" };

function variantOf(kit) {
  if (kit && VARIANTS[kit.variant]) return { id: kit.variant, ...VARIANTS[kit.variant] };
  const t = kit && kit.tone;
  if (t && RADIUS_PX[t.radius] && DENSITY_PAD[t.density]) {
    return { id: `tone:${t.radius}/${t.density}`, radius: RADIUS_PX[t.radius], btn: BTN_RADIUS[t.radius], padScale: DENSITY_PAD[t.density] };
  }
...
// The two remaining "cheap" tells, both measured in the current output:
//
//  * .eyebrow is coloured var(--accent) globally, so every single section
//    heading gets a gold kicker above it. Real luxury sites colour roughly 5%
//    of their text; a gold eyebrow everywhere is the clearest template signal
//    we ship.
//  * 31 text-align:center declarations. Symmetric centred stacks read as a
//    template; luxury composition is left-led and asymmetric, with centring
//    saved for one deliberate moment.
//
// Emitted LAST so it wins over the per-design blocks, which set both directly.
function toneCss(t) {
  if (!t) return "";
  // Restraint means the eyebrow stops being gold — it does NOT mean fading it.
  // Two earlier attempts were measured and rejected: var(--body) is right on a
  // light band but 2.41:1 on bold's dark ground, and adding opacity dragged a
  // valid 4.5:1 eyebrow down to 2.87:1. currentColor is safe by construction —
  // it takes the text colour the design already chose for that section, which is
  // readable there by definition.
  const eyebrow = { strong: "var(--accent)", moderate: "currentColor", minimal: "currentColor" }[t.accentIntensity];
  // A brand that asked for quiet, or for real space, gets the left-led setting.
  // Bold and dense brands keep the centred treatment, where it still reads.
  const leftLed = t.bandRhythm === "quiet" || t.density === "airy" || t.density === "expansive";
  // `.center p` (0-1-1) exists in several designs and outranks a bare `.eyebrow`
  // (0-1-0), so the section-header eyebrows silently kept their old colour.
  return `
  ${eyebrow ? `.eyebrow,.center .eyebrow,.center p.eyebrow{color:${eyebrow}}` : ""}
  ${leftLed ? `.center{text-align:left;margin-left:0}
  .center .eyebrow,.center h2,.center p{text-align:left}` : ""}`;
}
...
<style>
  :root{--cream:${t.cream || "#fffbf5"};--white:${t.white || "#ffffff"};--ink:${t.ink || "#323232"};--body:${t.body || "#585858"};--line:${t.line || "#e5ddd2"};--accent:${t.accent || "#8a7a63"};--serif:"${t.serifFont || "Cormorant Garamond"}",Georgia,serif;--sans:"${t.sansFont || "Montserrat"}",sans-serif;--wrap:1200px}
  ${SHARED}${d.css}${variantCss(v || variantOf(k))}${PREMIUM}${LUXE}${toneCss(k && k.tone)}
</style></head><body>`;
}

function header(k, d) {
  const b = k.brand || {};
  return `${d.topbar ? `<div${A("topbar")} class="topbar">${esc(b.topbar || b.city || "")}</div>` : ""}
  <header${A("header")}><nav class="nav wrap">
      <ul id="menu"><li><a${A("nav.home")} href="index.html">Home</a></li><li><a${A("nav.services")} href="services.html">Services</a></li><li><a${A("nav.about")} href="about.html">About</a></li><li><a${A("nav.contact")} href="contact.html">Contact</a></li></ul>
      <a${A("nav.logo")} class="logo" href="index.html">${esc(b.name || "MED SPA")}${d.logoSpan ? `<span>${esc(b.sub || "")}</span>` : ""}</a>
      <div class="nav-right"><a${A("nav.phone")} class="phone" href="tel:${esc(b.phone || "")}">${esc(b.phone || "")}</a><a${A("nav.cta")} class="btn" href="contact.html">${esc(d.book)}</a>
...
function footer(k) {
  ...
  const logo = b.logo
    ? `<img${A("footer.logo")} class="foot-logo" src="${esc(b.logo)}" alt="${esc(b.name || "")}" onerror="var d=document.createElement('div');d.className='logo';d.textContent='${esc(b.name || "")}';this.replaceWith(d)"/>`
    : `<div${A("footer.logo")} class="logo" style="margin-bottom:18px">${esc(b.name || "")}</div>`;
  ...
  const svc = arr(k.servicesMenu).slice(0, 5).map((x) => `<li><a href="services.html">${esc(x.name)}</a></li>`).join("") || `<li><a href="services.html">Services</a></li>`;
  const hours = k.contact && k.contact.hours;
  return `<footer${A("footer")}><div class="wrap"><div class="foot-top">
        <div class="foot-brand">${logo}<p${A("footer.blurb")}>${esc((k.footer && k.footer.blurb) || "")}</p>${socials ? `<div class="socials">${socials}</div>` : ""}</div>
        <div><h5>Explore</h5><ul><li><a href="index.html">Home</a></li><li><a href="about.html">About</a></li><li><a href="services.html">Services</a></li><li><a href="contact.html">Contact</a></li></ul></div>
        ...
        <div><h5>Visit Us</h5><ul>${b.city ? `<li>${esc(b.city)}</li>` : ""}${b.phone ? `<li><a href="tel:${esc(b.phone)}">${esc(b.phone)}</a></li>` : ""}${b.email ? `<li><a href="mailto:${esc(b.email)}">${esc(b.email)}</a></li>` : ""}${hours ? `<li>${esc(hours)}</li>` : ""}</ul>
          <a class="btn btn--light" style="margin-top:20px" href="contact.html">Book a Visit</a></div></div>
...
function heroSection(k, d) {
  const h = k.hero || {}, img = h.image ? cover(h.image, "hero.image") : "";
  if (d.hero === "split") {
    ...
    return `<section${A("hero")} class="hero hero--split"><div class="wrap hero-grid">
        <div class="hero-copy"><p${A("hero.eyebrow")} class="eyebrow">${esc(h.eyebrow)}</p><h1${A("hero.h1")}>${esc(h.h1)}</h1>
          <p${A("hero.body")} class="hero-lead">${esc(h.body)}</p>
          <div class="hero-actions"><a${A("hero.cta")} class="btn" href="contact.html">${esc(h.cta || "Book a Visit")}</a>
            <a${A("hero.cta2")} class="hero-alt" href="services.html">Explore treatments</a></div></div>
        <div class="hero-media"><div class="hero-frame"></div>${img}</div>
      </div></section>`;
  }
  if (d.hero === "centered") {
    return `<section${A("hero")} class="hero"><div class="wrap"><p${A("hero.eyebrow")} class="eyebrow">${esc(h.eyebrow)}</p><h1${A("hero.h1")}>${esc(h.h1)}</h1><p${A("hero.body")}>${esc(h.body)}</p>
        <a${A("hero.cta")} class="btn" href="contact.html">${esc(h.cta || "Book a Visit")}</a></div>${img ? `<div class="hero-media">${img}</div>` : ""}</section>`;
  }
  // overlay (editorial) + bottom (bold) share markup; CSS differs
  return `<section${A("hero")} class="hero"><div class="hero-bg">${img}</div><div class="wrap">
      <p${A("hero.eyebrow")} class="eyebrow">${esc(h.eyebrow)}</p><h1${A("hero.h1")}>${esc(h.h1)}</h1><p${A("hero.body")}>${esc(h.body)}</p>
      <a${A("hero.cta")} class="btn btn--light" href="contact.html">${esc(h.cta || "Book a Visit")}</a></div></section>`;
}
function pageHero(k, intro, title) {
  const i = intro || {};
  return `<section${A("pagehero")} class="page-hero"><div class="page-hero-bg">${i.image ? cover(i.image, "pagehero.image") : ""}</div><div class="wrap">
      <p${A("pagehero.eyebrow")} class="eyebrow">${esc(i.eyebrow || (k.brand && k.brand.name) || "")}</p><h1${A("pagehero.h1")}>${esc(i.h1 || title)}</h1>
      ${i.body ? `<p${A("pagehero.body")} style="max-width:600px;margin:18px auto 0;color:#f2ece2">${esc(i.body)}</p>` : ""}</div></section>`;
}
function aboutSection(k, d) {
  ...
  return `<section${A("about")} class="${cls}" id="about"><div class="wrap about-grid${hasImg ? "" : " solo"}">
      <div class="about-copy"><p${A("about.eyebrow")} class="eyebrow">${esc(a.eyebrow)}</p><h2${A("about.h2")}>${esc(a.h2)}</h2>
        <div${A("about.body")} style="margin-top:24px">${paras}</div><a${A("about.cta")} class="btn" href="contact.html">${esc(a.cta || "Book Now")}</a></div>
      ${hasImg ? `<div class="about-img">${cover(a.image, "about.image")}</div>` : ""}</div></section>`;
}
function stripSection(k, d) {
  const s = arr(k.strip);
  if (!s.length || d.strip === "none") return "";
  if (d.strip === "marquee") {
    const items = s.map((x, i) => `<span${A(`strip.items.${i}`)}>${esc(x)} &nbsp;·</span>`).join("");
    // The row is repeated for a seamless loop; only the first copy is anchored,
    // so an override targets one item rather than two clones of it.
    const dup = s.map((x) => `<span>${esc(x)} &nbsp;·</span>`).join("");
    return `<div${A("strip")} class="strip"><div class="track">${items}${dup}</div></div>`;
  }
  return `<div${A("strip")} class="strip"><div class="wrap">${s.map((x, i) => `<span${A(`strip.items.${i}`)}>${esc(x)}</span>`).join("")}</div></div>`;
}
function specialtiesSection(k, d) {
  const sp = k.specialties || {};
  const cards = arr(sp.cards).map((c, i) => `
      <div${A(`specialties.cards.${i}`)} class="card${c.image ? "" : " card--text"}">${d.cardIdx ? `<span class="idx">0${i + 1}</span>` : ""}${c.image ? `<div class="card-img">${cover(c.image, `specialties.cards.${i}.image`)}</div>` : ""}<div class="card-body">
        <h3${A(`specialties.cards.${i}.h3`)}>${esc(c.h3)}</h3><p${A(`specialties.cards.${i}.p`)}>${esc(c.p)}</p><a${A(`specialties.cards.${i}.cta`)} class="btn" href="services.html">Learn More</a></div></div>`).join("");
  return `<section${A("specialties")} class="section" id="specialties"><div class="wrap"><div class="center">${cx(d)}<p${A("specialties.eyebrow")} class="eyebrow">${esc(sp.eyebrow)}</p><h2${A("specialties.h2")}>${esc(sp.h2)}</h2>${sp.intro ? `<p${A("specialties.intro")}>${esc(sp.intro)}</p>` : ""}</div>
...
function providersSection(k, d) {
  ...
  const html = members.map((m, i) => `
      <div${A(`providers.members.${i}`)} class="member">${m.image ? `<div class="photo">${cover(m.image, `providers.members.${i}.image`)}</div>` : ""}<h3${A(`providers.members.${i}.name`)}>${esc(m.name)}</h3><div${A(`providers.members.${i}.role`)} class="role">${esc(m.role)}</div></div>`).join("");
  return `<section${A("providers")} class="section providers" id="providers"><div class="wrap"><div class="center">${cx(d)}<p${A("providers.eyebrow")} class="eyebrow">${esc(pr.eyebrow)}</p><h2${A("providers.h2")}>${esc(pr.h2)}</h2></div>
...
function testimonialsSection(k, d) {
  ...
  if (!q.length) return "";
  const html = q.map((x, i) => `<div${A(`testimonials.quotes.${i}`)} class="quote"><h4${A(`testimonials.quotes.${i}.h4`)}>${esc(x.h4)}</h4><p${A(`testimonials.quotes.${i}.p`)}>"${esc(x.p)}"</p><cite${A(`testimonials.quotes.${i}.cite`)}>— ${esc(x.cite)}</cite></div>`).join("");
  ...
  const oneStyle = q.length === 1
    ...
    : "";
  return `<section${A("testimonials")} class="section testi" id="testi"><div class="wrap"><div class="center">${cx(d)}<p${A("testimonials.eyebrow")} class="eyebrow">${esc(t.eyebrow)}</p><h2${A("testimonials.h2")}>${esc(t.h2)}</h2></div><div class="quotes${q.length === 1 ? " quotes--one" : ""}"${oneStyle}>${html}</div></div></section>`;
}
function featuredSection(k, d) {
  const f = k.featured || {}, items = arr(f.items);
  if (!items.length) return "";
  const html = items.map((x, i) => `<article${A(`featured.items.${i}`)}><h3${A(`featured.items.${i}.h3`)}>${esc(x.h3)}</h3><p${A(`featured.items.${i}.p`)}>${esc(x.p)}</p><a${A(`featured.items.${i}.cta`)} class="btn" href="contact.html">Book Now</a></article>`).join("");
  return `<section${A("featured")} class="section"><div class="wrap"><div class="center">${cx(d)}<h2${A("featured.h2")}>${esc(f.h2)}</h2></div><div class="feat">${html}</div></div></section>`;
}
function ctaSection(k, d) {
  const c = k.cta || {}, b = k.brand || {};
  const btnLight = d.hero === "bottom" ? " btn--light" : "";
  return `<section${A("cta")} class="cta" id="cta"><div class="wrap">${cx(d)}<p${A("cta.eyebrow")} class="eyebrow">${esc(c.eyebrow)}</p><h2${A("cta.h2")}>${esc(c.h2)}</h2><p${A("cta.body")}>${esc(c.body)}</p>
      <a${A("cta.cta")} class="btn${btnLight}" href="tel:${esc(b.phone || "")}">Contact Us</a><div${A("cta.contactline")} class="contactline">${esc(b.phone || "")}${b.email ? " · " + esc(b.email) : ""}</div></div></section>`;
}

function gallerySection(k, d) {
  const g = arr(k.gallery);
  if (g.length < 2) return ""; // only show when there are real extra photos
  const cells = g.map((u, i) => `<div class="gcell">${cover(u, `gallery.images.${i}`)}</div>`).join("");
  return `<section${A("gallery")} class="section gallery-sec"><div class="wrap"><div class="center">${cx(d)}<p${A("gallery.eyebrow")} class="eyebrow">Gallery</p><h2${A("gallery.h2")}>A Look Inside</h2></div><div class="gallery">${cells}</div></div></section>`;
}
...
function renderHome(kit) {
  ...
  return mirrorAnchorsToClasses(head(kit, d) + header(kit, d) + heroSection(kit, d) + aboutSection(kit, d) + stripSection(kit, d)
...
function servicesMenuSection(k, d) {
  ...
  const rows = m.map((x, i) => `<article${A(`servicesMenu.rows.${i}`)} class="svc-row"><span class="svc-n">${String(i + 1).padStart(2, "0")}</span><div><h3${A(`servicesMenu.rows.${i}.name`)}>${esc(x.name)}</h3><p${A(`servicesMenu.rows.${i}.desc`)}>${esc(x.desc)}</p></div><a${A(`servicesMenu.rows.${i}.cta`)} class="btn" href="contact.html">Book</a></article>`).join("");
  return `<section${A("servicesMenu")} class="section"><div class="wrap"><div class="center">${cx(d)}<p${A("servicesMenu.eyebrow")} class="eyebrow">${esc(sp.eyebrow || "Our Services")}</p><h2${A("servicesMenu.h2")}>Treatments &amp; Services</h2>${sp.body ? `<p${A("servicesMenu.body")}>${esc(sp.body)}</p>` : ""}</div><div class="svc-menu">${rows}</div></div></section>`;
}
function valuesSection(k, d) {
  const v = arr(k.values); if (!v.length) return "";
  const items = v.map((x, i) => `<div${A(`values.items.${i}`)} class="value"><h3${A(`values.items.${i}.h3`)}>${esc(x.h3)}</h3><p${A(`values.items.${i}.p`)}>${esc(x.p)}</p></div>`).join("");
  return `<section${A("values")} class="section"><div class="wrap"><div class="center">${cx(d)}<p${A("values.eyebrow")} class="eyebrow">Why ${esc((k.brand && k.brand.name) || "Us")}</p><h2${A("values.h2")}>The difference</h2></div><div class="values">${items}</div></div></section>`;
}
...
function contactPremium(k, d, next) {
  ...
  const photo = next();
  const ci = (l, v, href, ic, anchor) => v ? `<div${A(anchor)} class="ci-card"><span class="ci-ic">${ic}</span><div><dt>${l}</dt><dd>${href ? `<a href="${href}">${esc(v)}</a>` : esc(v)}</dd></div></div>` : "";
  ...
  return `<section${A("contact")} class="section about"><div class="wrap"><div class="about-grid">
      ${photo ? `<div class="about-img">${cover(photo, "contact.image")}</div>` : ""}
      <div class="about-copy"><p${A("contact.eyebrow")} class="eyebrow">Plan Your Visit</p><h2${A("contact.h2")}>${esc(cp.h2 || "We can't wait to welcome you")}</h2>
        <p${A("contact.body")}>${esc(cp.body || "Reach out to schedule a private consultation — our concierge team will help you find the perfect time to visit.")}</p>
        <div class="ci-grid">${ci("Address", c.address, null, "◈", "contact.address")}${ci("Phone", c.phone, "tel:" + esc(c.phone), "☎", "contact.phone")}${ci("Email", c.email, "mailto:" + esc(c.email), "✉", "contact.email")}${ci("Hours", c.hours, null, "◷", "contact.hours")}</div>
        <a${A("contact.cta")} class="btn" style="margin-top:30px" href="tel:${esc(c.phone || "")}">${esc(c.booking || "Book a consultation")}</a></div>
    </div></div></section>${map}`;
}
function renderServices(kit) {
  const d = designOf(kit);
  return mirrorAnchorsToClasses(head(kit, d) + header(kit, d) + pageHero(kit, kit.servicesPage, "Our Services")
...
function renderAbout(kit) {
  ...
  return mirrorAnchorsToClasses(head(kit, d) + header(kit, d) + pageHero(kit, kit.aboutPage, "About Us")
...
function renderContact(kit, pool) {
  ...
  return mirrorAnchorsToClasses(head(kit, d) + header(kit, d) + pageHero(kit, hero, "Contact Us")
...
// Blueprint blocks carry their ordinal in the anchor (`block.2.h2`), because
// unlike the fixed sections they have no inherent role name — a page can hold
// several `split` blocks. The ordinal is the client's own scraped section order,
// so it is as stable as their site's structure.
function blockSplit(b, d, ns) {
  const paras = arr(b.paras).map((p, i) => `<p${A(`${ns}.paras.${i}`)}>${esc(p)}</p>`).join("");
  const copy = `<div class="about-copy"><p${A(`${ns}.eyebrow`)} class="eyebrow">${esc(b.eyebrow)}</p><h2${A(`${ns}.h2`)}>${esc(b.h2)}</h2>${paras}</div>`;
  const img = b.image ? `<div class="about-img">${cover(b.image, `${ns}.image`)}</div>` : "";
  const inner = img ? (b.reverse ? copy + img : img + copy) : copy;
  return `<section${A(ns)} class="section about"><div class="wrap"><div class="about-grid${img ? "" : " solo"}">${inner}</div></div></section>`;
}
function blockCards(b, d, ns) {
  const cs = arr(b.cards); if (!cs.length) return "";
  const html = cs.map((c, i) => `<div${A(`${ns}.cards.${i}`)} class="card">${c.image ? `<div class="card-img">${cover(c.image, `${ns}.cards.${i}.image`)}</div>` : ""}<div class="card-body"><h3${A(`${ns}.cards.${i}.h3`)}>${esc(c.h3)}</h3><p${A(`${ns}.cards.${i}.p`)}>${esc(c.p)}</p><a${A(`${ns}.cards.${i}.cta`)} class="btn" href="contact.html">Learn More</a></div></div>`).join("");
  return `<section${A(ns)} class="section"><div class="wrap"><div class="center">${cx(d)}<p${A(`${ns}.eyebrow`)} class="eyebrow">${esc(b.eyebrow)}</p><h2${A(`${ns}.h2`)}>${esc(b.h2)}</h2>${b.intro ? `<p${A(`${ns}.intro`)}>${esc(b.intro)}</p>` : ""}</div><div class="cards">${html}</div></div></section>`;
}
function blockList(b, d, ns) {
  const it = arr(b.items); if (!it.length) return "";
  const rows = it.map((x, i) => `<article${A(`${ns}.items.${i}`)} class="svc-row"><span class="svc-n">${String(i + 1).padStart(2, "0")}</span><div><h3${A(`${ns}.items.${i}.name`)}>${esc(x.name)}</h3><p${A(`${ns}.items.${i}.desc`)}>${esc(x.desc)}</p></div><a${A(`${ns}.items.${i}.cta`)} class="btn" href="contact.html">Book</a></article>`).join("");
  return `<section${A(ns)} class="section"><div class="wrap"><div class="center">${cx(d)}<p${A(`${ns}.eyebrow`)} class="eyebrow">${esc(b.eyebrow)}</p><h2${A(`${ns}.h2`)}>${esc(b.h2)}</h2>${b.intro ? `<p${A(`${ns}.intro`)}>${esc(b.intro)}</p>` : ""}</div><div class="svc-menu">${rows}</div></div></section>`;
}
function blockGallery(b, d, ns) {
  const im = arr(b.images).filter(Boolean); if (!im.length) return "";
  const cells = im.slice(0, 6).map((u, i) => `<div class="gcell">${cover(u, `${ns}.images.${i}`)}</div>`).join("");
  return `<section${A(ns)} class="section"><div class="wrap">${(b.h2 || b.eyebrow) ? `<div class="center">${cx(d)}<p${A(`${ns}.eyebrow`)} class="eyebrow">${esc(b.eyebrow)}</p><h2${A(`${ns}.h2`)}>${esc(b.h2)}</h2></div>` : ""}<div class="gallery">${cells}</div></div></section>`;
}
function blockQuote(b, d, ns) {
  const qs = arr(b.quotes); if (!qs.length) return "";
  const html = qs.map((x, i) => `<div${A(`${ns}.quotes.${i}`)} class="quote"><h4${A(`${ns}.quotes.${i}.h4`)}>${esc(x.h4)}</h4><p${A(`${ns}.quotes.${i}.p`)}>"${esc(x.p)}"</p><cite${A(`${ns}.quotes.${i}.cite`)}>— ${esc(x.cite)}</cite></div>`).join("");
  const one = qs.length === 1 ? ` style="grid-template-columns:minmax(0,660px);justify-content:center;text-align:center"` : "";
  return `<section${A(ns)} class="section testi"><div class="wrap"><div class="center">${cx(d)}<p${A(`${ns}.eyebrow`)} class="eyebrow">${esc(b.eyebrow)}</p><h2${A(`${ns}.h2`)}>${esc(b.h2)}</h2></div><div class="quotes${qs.length === 1 ? " quotes--one" : ""}"${one}>${html}</div></div></section>`;
}
function blockStat(b, d, ns) {
  const it = arr(b.items).filter((x) => x && (x.big || x.label)); if (!it.length) return "";
  const cells = it.slice(0, 4).map((x, i) => `<div${A(`${ns}.items.${i}`)} class="value"><h3${A(`${ns}.items.${i}.big`)} style="color:var(--accent)">${esc(x.big)}</h3><p${A(`${ns}.items.${i}.label`)} style="letter-spacing:1px;text-transform:uppercase;font-size:12px">${esc(x.label)}</p></div>`).join("");
  return `<section${A(ns)} class="section"><div class="wrap"><div class="values">${cells}</div></div></section>`;
}
function blockHtml(b, d, k, ns) {
  switch (b.type) {
    case "split": return blockSplit(b, d, ns);
    case "cards": return blockCards(b, d, ns);
    case "list": return blockList(b, d, ns);
    case "gallery": return blockGallery(b, d, ns);
    case "quote": return blockQuote(b, d, ns);
    case "stat": return blockStat(b, d, ns);
    // cta reuses the fixed section, so it keeps the fixed `cta.*` anchors rather
    // than a block ordinal — a page has one closing CTA whichever path built it.
...
function renderFromBlueprint(kit, bp, title, pool) {
  ...
  const body = arr(bp.blocks).map((b, i) => blockHtml(b, d, kit, `block.${i}`)).join("");
  return mirrorAnchorsToClasses(head(kit, d) + header(kit, d) + pageHero(kit, hero, title) + `<main>` + body + `</main>` + footer(kit) + MOTION_JS + "</body></html>");
}
