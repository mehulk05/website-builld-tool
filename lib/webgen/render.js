...
document.querySelectorAll('section,footer').forEach(function(sec){var kids=sec.querySelectorAll('.card,.member,.quote,.feat article,.about-copy,.about-img,.center,.hero .wrap>*');var g=kids.length?kids:[sec];g.forEach(function(el,i){if(rm)return;el.setAttribute('data-reveal','');el.style.transitionDelay=(Math.min(i,6)*90)+'ms';io.observe(el);});});
var h=document.querySelector('header');addEventListener('scroll',function(){if(h)h.classList.toggle('shrink',(scrollY||0)>20);},{passive:true});
...
function header(k, d) {
  ...
  <header><nav class="nav wrap">
      <ul id="menu"><li><a href="/">Home</a></li><li><a href="/services/">Services</a></li><li><a href="/about/">About</a></li><li><a href="/team/">Team</a></li></ul>
      <a class="logo" href="/">${esc(b.name || "MED SPA")}${d.logoSpan ? `<span>${esc(b.sub || "")}</span>` : ""}</a>
      <div class="nav-right"><a class="phone" href="tel:${esc(b.phone || "")}">${esc(b.phone || "")}</a><a class="btn" href="/services/">${esc(d.book)}</a>
...
function footer(k) {
  ...
  return `<footer><div class="wrap"><div class="foot-grid">
        <div><div class="logo" style="margin-bottom:18px">${esc(b.name || "")}</div><p>${esc(k.footer && k.footer.blurb)}</p></div>
        <div><h5>Explore</h5><ul><li><a href="/about/">About</a></li><li><a href="/services/">Services</a></li><li><a href="/team/">Team</a></li></ul></div>
        <div><h5>Contact</h5><ul><li><a href="tel:${esc(b.phone || "")}">${esc(b.phone || "")}</a></li>${b.email ? `<li><a href="mailto:${esc(b.email)}">${esc(b.email)}</a></li>` : ""}</ul></div>
        <div><h5>Visit</h5><ul><li>${esc(b.city || "")}</li></ul></div></div>
...
function heroSection(k, d) {
  ...
  if (d.hero === "centered") {
    return `<section class="hero"><div class="wrap"><p class="eyebrow">${esc(h.eyebrow)}</p><h1>${esc(h.h1)}</h1><p>${esc(h.body)}</p>
        <a class="btn" href="/services/">${esc(h.cta || "Book a Visit")}</a></div>${img ? `<div class="hero-media">${img}</div>` : ""}</section>`;
  }
  ...
  return `<section class="hero"><div class="hero-bg">${img}</div><div class="wrap">
      ...
      <a class="btn btn--light" href="/services/">${esc(h.cta || "Book a Visit")}</a></div></section>`;
}
...
function aboutSection(k, d) {
  ...
  return `<section class="${cls}" id="about"><div class="wrap about-grid${hasImg ? "" : " solo"}">
      <div class="about-copy"><p class="eyebrow">${esc(a.eyebrow)}</p><h2>${esc(a.h2)}</h2>
        <div style="margin-top:24px">${paras}</div><a class="btn" href="/services/">${esc(a.cta || "Book Now")}</a></div>
...
function testimonialsSection(k, d) {
  ...
  return `<section class="section testi" id="testi"><div class="wrap"><div class="center">${cx(d)}<p class="eyebrow">${esc(t.eyebrow)}</p><h2>${esc(t.h2)}</h2></div><div class="quotes">${html}</div></div></section>`;
}
function featuredSection(k, d) {
  const f = k.featured || {}, items = arr(f.items);
  if (!items.length) return "";
  const html = items.map((x) => `<article><h3>${esc(x.h3)}</h3><p>${esc(x.p)}</p><a class="btn" href="/services/">Book Now</a></article>`).join("");
...
function renderServices(kit) {
  ...
  return head(kit, d) + header(kit, d) + pageHero(kit, kit.servicesPage, "Our Services")
    + `<main>` + specialtiesSection(kit, d) + featuredSection(kit, d) + ctaSection(kit, d) + `</main>` + footer(kit) + MOTION_JS + "</body></html>";
}
function renderAbout(kit) {
  const d = designOf(kit);
  return head(kit, d) + header(kit, d) + pageHero(kit, kit.aboutPage, "About Us")
    + `<main>` + aboutSection(kit, d) + stripSection(kit, d) + gallerySection(kit, d) + testimonialsSection(kit, d) + ctaSection(kit, d) + `</main>` + footer(kit) + MOTION_JS + "</body></html>";
}
function renderTeam(kit) {
  const d = designOf(kit);
  const providers = providersSection(kit, d) || `<section class="section"><div class="wrap"><div class="center">${cx(d)}<h2>${esc((kit.providers && kit.providers.h2) || "Our Team")}</h2></div></div></section>`;
  return head(kit, d) + header(kit, d) + pageHero(kit, kit.teamPage, "Meet the Team")
    + `<main>` + providers + ctaSection(kit, d) + `</main>` + footer(kit) + MOTION_JS + "</body></html>";
}
function renderPages(kit) {
  return { home: renderHome(kit), services: renderServices(kit), about: renderAbout(kit), team: renderTeam(kit) };
}

module.exports = { renderPages, renderHome, renderServices, renderAbout, renderTeam, DESIGN_IDS: Object.keys(DESIGNS) };
