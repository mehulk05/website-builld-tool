...
function sanitizeImages(kit, pool, labels = new Map()) {
  ...
  const labelOf = (im) => {
    ...
    if (im.kind === "portrait") return "person"; // scrape's alt-based portrait detection
    return NAME.test(file(im.src)) && !NOTPERSON.test(file(im.src)) ? "person" : "photo";
  };
  const BGFILE = /(^|[-_/.])(bg|background|banner|cta|section|hero|overlay|backdrop)([-_/.]|$)/i;
  ...
  for (const im of [...pool].sort((a, b) => (b.w || 0) - (a.w || 0))) { // biggest first → hero
    ...
    else if (l === "person") person.push(im.src);
    else photo.push(im.src);
  }
  const used = new Set();
  const take = (list) => { for (const s of list) if (!used.has(s)) { used.add(s); return s; } return ""; };

  const STOP = /^(headshot|portrait|profile|photo|image|img|pic|scaled|final|edit|edited|cropped|copy|web|webp|min|small|large|new|team|staff|about|our|the|and|of|dr|nuvo|ruma|aesthetics?|clinic|medical|med|spa|wellness|beauty|sycamore|lehi|utah|il|ut|us|inc|llc)$/i;
  const cap1 = (w) => w[0].toUpperCase() + w.slice(1).toLowerCase();
  const nameFromFile = (src) => {
    const base = decodeURIComponent(file(src)).replace(/​/g, "").replace(/\.[a-z0-9]+$/i, "");
    const parts = base.split(/[-_ .]+/).filter((w) => w && /[A-Za-z]/.test(w) && !/^\d+$/.test(w) && !STOP.test(w) && !/^[a-z0-9]{8,}$/.test(w));
    return parts.slice(0, 2).map(cap1).join(" ").trim();
  };
  const realName = (n) => /^[A-Z][a-z]+ [A-Z][a-z]+$/.test((n || "").trim()) && !/headshot|assistant|specialist|aesthetician|provider|medical|team/i.test(n);
  const members = kit.providers && kit.providers.members || [];
  const seen = new Set();
  const built = [];
  for (const src of person) {
    const name = nameFromFile(src) || (realName(members[built.length] && members[built.length].name) ? members[built.length].name : "");
  ...
  kit.providers.members = built;

  if (kit.hero) kit.hero.image = take(photo) || take(decor) || "";
...
async function classifyPool(pool, apiKey, modelName) {
  try { return await classifyImages(pool, makeModel(apiKey, modelName)); }
...
function enrichKitImages(kit, pool, labels) {
  ...
  for (const c of (kit.specialties && kit.specialties.cards) || []) c.image = take(photo);
  if (kit.about) kit.about.image = take(photo);
  const mem = (kit.providers && kit.providers.members) || [];
  if (mem.length) mem.forEach((m, i) => { m.image = person[i] || ""; });
