...
function rgbOf(hex) { const m = String(hex).replace("#", ""); if (m.length !== 6) return [42, 42, 42]; return [parseInt(m.slice(0, 2), 16), parseInt(m.slice(2, 4), 16), parseInt(m.slice(4, 6), 16)]; }
function toHex(a) { const h = (x) => Math.max(0, Math.min(255, Math.round(x))).toString(16).padStart(2, "0"); return "#" + h(a[0]) + h(a[1]) + h(a[2]); }
function lum(hex) { const [r, g, b] = rgbOf(hex); return 0.299 * r + 0.587 * g + 0.114 * b; }
function mix(a, b, t) { const A = rgbOf(a), B = rgbOf(b); return toHex([A[0] + (B[0] - A[0]) * t, A[1] + (B[1] - A[1]) * t, A[2] + (B[2] - A[2]) * t]); }

// Map the confirmed step-2 palette → a FULL token set. Heroes, overlays, footers,
// on-dark text and borders all DERIVE from the brand ink/accent/cream so the whole
// design adapts (no baked ruma-warm colours) and stays contrast-safe.
function themeTokens(composed = {}) {
  const cands = [composed.primary, composed.secondary].filter(Boolean);
  const ink = cands.find((c) => lum(c) < 110) || "#232323"; // contrast guard: a genuinely dark brand colour
  let accent = composed.accent || composed.secondary || "#8a7a63";
  if (lum(accent) > 168) accent = mix(accent, ink, 0.42); // pale accent → darken so eyebrows/borders/buttons stay readable on light bg
  const onAccent = lum(accent) > 150 ? ink : "#ffffff";   // readable text ON an accent fill (button hover)
  const cream = composed.primary && isLight(composed.primary) ? composed.primary : "#fffbf5";
  const [ir, ig, ib] = rgbOf(ink);
  return {
    cream, white: "#ffffff", ink,
    body: mix(ink, "#ffffff", 0.30),        // readable body grey derived from ink
    line: mix(cream, ink, 0.14),            // subtle border tied to the palette
    accent,
    ...
    // derived tokens — heroes/overlays/footers now follow the brand, not baked warm
    heroA: ink,
    heroB: mix(ink, accent, 0.32),          // hero gradient lifts toward the brand accent
    overlay: `rgba(${ir},${ig},${ib},.55)`, // dark scrim from brand ink
    onDark: "#f5f1ea",                       // soft neutral text on dark (works on any brand)
    onDarkMuted: "#c9c3ba",
    footBg: mix(ink, "#000000", 0.35),       // footer = deep brand ink
    footFg: mix(ink, "#ffffff", 0.55),
    lineDark: "rgba(255,255,255,.14)",       // borders on dark sections
    onAccent,                                // readable text on an accent fill
  };
}
