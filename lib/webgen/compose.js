...
Return ONLY JSON (no markdown) in EXACTLY this shape:
{
 ...
 "layout":"editorial | bold | minimal | aura"
}
...
Pick from the brand's palette + tone (dark/edgy → bold; classic → editorial; clean/calm → minimal; warm/soft → aura).
RULES:
...
${siteSummary(siteStruct) || "(no scan available — compose from onboarding only)"}`;
    ...
    try {
      ...
      kit = {
        ...
        layout: ["editorial", "bold", "minimal", "aura"].includes(ai.layout) ? ai.layout : fb.layout,
      };
    } catch (e) {
