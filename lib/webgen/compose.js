...
function themeTokens(composed = {}) {
  ...
  return {
    ...
    // accent must carry visual weight: a near-white extracted accent (NUVO's #F4EFEA)
    // washed out every gold hairline, eyebrow and CTA. Too light → warm bronze default.
    accent: (composed.accent && !isLight(composed.accent)) ? composed.accent
      : ((composed.secondary && !isLight(composed.secondary)) ? composed.secondary : "#8a7a63"),
