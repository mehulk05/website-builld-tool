...
const COUNTED = ["img", "a", "h1", "h2", "h3", "form", "input", "video", "iframe"];
...
function checkAddressed(before, after, css, items, addressedIds, containerId) {
  ...
  for (const it of items) {
    if (!claimed.has(String(it.localId)) && !claimed.has(String(it.id))) {
...
module.exports = {
  ...
  checkHtml, checkCss, checkAddressed, deadCssRules, countTags, elementCount,
};
