...
async function patchSection({ html, items, containerId, page, ai }) {
  ...
  for (const it of items) {
    ...
    if (d) { working = d.html; doneIds.push(it.localId); continue; }
    // A note carrying a file that the deterministic path could not place must
    // NOT go to the model.
    //
    // The model has no way to use an upload — it never sees the URL — so asked
    // to swap a picture it cannot swap, it writes a plausible filename instead.
    // That happened on a live site: a photo attached to a band holding four of
    // them, reported as applied, the reviewer's file ignored, and a founder's
    // headshot left pointing at a URL nobody had ever created. The page still
    // passed the render check, because the host answered the missing file with
    // its 404 page under HTTP 200.
    //
    // There is exactly one thing worth saying here, and it is a question.
    if (it.imageUrl) { undecided.push(it); continue; }
    remaining.push(it);
  }

  // How many pictures are actually there, so the refusal can say so rather than
  // leaving the reviewer to count them.
  const pictureCount = (working.match(/<img\b/gi) || []).length;
  const cannotPlace = (it) => ({
    item: it,
    ok: false,
    reason: pictureCount > 1
      ? `there are ${pictureCount} pictures in this section, so it is not clear which one to replace — leave the note on the picture itself and it will be swapped`
      : "the picture could not be placed in this section",
  });

  if (!remaining.length) {
    return {
      html: working, css: "", addressed: doneIds, usedModel: false, modelNotes: "",
      verdicts: items.map((it) => (undecided.includes(it) ? cannotPlace(it) : { item: it, ok: true })),
    };
  }
  ...
  if (!htmlCheck.ok) {
    return {
      ...
      verdicts: items.map((it) => (doneIds.includes(it.localId)
        ? { item: it, ok: true }
        : undecided.includes(it) ? cannotPlace(it)
  ...
  // A note whose picture could not be placed is refused with its own reason,
  // never with checkAddressed's — the model was not asked about it and cannot
  // have addressed it.
  const verdicts = V.checkAddressed(working, out.html, out.css, items, claimed, containerId)
    .map((v) => (doneIds.includes(v.item.localId) ? { item: v.item, ok: true }
      : undecided.includes(v.item) ? cannotPlace(v.item)
      : v));
