...
async function patchSection({ html, items, containerId, page, ai }) {
  ...
  const allowStructural = remaining.some((it) => /\b(remove|delete|drop|add|insert|extra|another|duplicate|reorder|move|swap|split|merge)\b/i.test(it.note));
...
module.exports = { patchSection, deterministicEdit, swapImage, buildPrompt, parseReply, MAX_FRAGMENT };
