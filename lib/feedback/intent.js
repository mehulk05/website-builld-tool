...
// A wider reading of "change what is there", used ONLY to decide whether the
// element counts are allowed to move.
//
// Deliberately looser than REMOVE, and deliberately not shared with classify().
// The two decisions are not the same size: the classifier picks an operation and
// can take a whole band off the page, so it stays strict; this only unlocks a
// safety check on a rewrite the model has already produced, so being generous
// costs a check and being stingy costs the reviewer their note.
//
// "take the photo out" is the case that made the difference — the object sits
// between the verb and the particle, which REMOVE's contiguous "take out" never
// sees.
const STRUCTURAL =
  /\b(?:remove|delete|drop|get rid of|add|insert|append|include|extra|another|duplicate|reorder|move|swap|split|merge|replace)\b|\btake\b(?:\s+\S+){0,4}\s+\bout\b/i;

/**
 * Does this note ask for something to be added or taken away?
 *
 * The structural guard in patch.js kept its own word list, and the two lists
 * disagreed: "take the photo out from the bottom of this section" is a removal
 * to the classifier and was not one to the guard, so a note that asked for
 * exactly what happened came back as "the feedback did not ask to remove". A
 * reviewer told that about their own sentence has no way to act on it.
 *
 * Negations are masked first, so "don't remove the hero" does not hand out
 * permission to remove things.
 */
function asksForStructure(note) {
  return STRUCTURAL.test(maskNegations(note));
}

module.exports = {
  maskNegations, NEGATED_REMOVE, asksForStructure, STRUCTURAL, REMOVE, ADD, MOVE, classify, route };
