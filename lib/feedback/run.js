...
async function applySiteChange({ root, entry, ai, log }) {
  ...
  if (intent.kind === "sitecss") {
    ...
    // item.imageUrl cannot be set here any more — such notes are routed to the
    // section pipeline before any classifier sees them — but the branch stays
    ...
    no(item.imageUrl
      ...
      : "no image was attached to this note. Use “Add a picture” in the note box and the file will be placed for you.");
...
async function applyNotes({ root, items, ai, liveUrl, log = () => {} }) {
  ...
  for (const item of items) {
    // A note carrying a file is not a classification problem.
    //
    // The reviewer chose a picture and chose where it goes; there is nothing
    // left to read. Sending it to the classifier anyway meant the answer
    // "image" — the most natural thing a model can say about a note with an
    // image attached — routed it to the branch that refuses image notes for
    // having no image attached. It is a section edit, and patch.js applies it
    // without a model at all: swapping the one picture there, or placing one
    // where the band had none.
    if (item.imageUrl) { routed.section.push(item); continue; }
