...
//
// A path rather than a constant, because the default is wrong wherever this
// runs on a host that replaces its disk between releases. On render it does,
// and every picture a reviewer had attached went with it — a photo added to a
// client's page on Tuesday was a 404 by Thursday, and the page still pointed
// at it.
//
// FEEDBACK_UPLOAD_DIR points this at storage that survives a deploy. It is
// deliberately just a directory: nothing else in this file cares where it is,
// and turning it on is a host setting rather than a release.
//
// This is a stopgap either way. A picture served from the build tool depends
// on the build tool being reachable forever, and it belongs in the client's
// own media library — see INFRA-media-on-deploy.md.
const DIR = process.env.FEEDBACK_UPLOAD_DIR
  ? path.resolve(process.env.FEEDBACK_UPLOAD_DIR)
  : path.join(__dirname, "..", "..", "generated", "feedback-uploads");
