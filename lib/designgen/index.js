...
function unifyChrome(pages, siteDir, log) {
  ...
  const homeHeader = extract(home, "header");
  const homeFooter = extract(home, "footer");
  if (!homeHeader && !homeFooter) return;
  ...
  for (const [key, html] of Object.entries(pages)) {
    ...
    let out = html;
    if (homeHeader) {
      const pageHeader = extract(out, "header");
      if (pageHeader) {
        ...
        if (activeHref) {
          const homeNav = extract(header, "nav");
          if (homeNav) {
            ...
            header = header.replace(homeNav, navOut);
          }
        }
        out = out.replace(pageHeader, header);
      }
    }
    if (homeFooter) {
      const pageFooter = extract(out, "footer");
      if (pageFooter) out = out.replace(pageFooter, homeFooter);
    }
...
Extra:   .c-btn (+ --ghost / --square) inside .c-btns · .c-book-tab (fixed side booking tab)
