...
async function gemini(parts, { temperature = 0.6, maxTokens = 20000, models = MODELS, waves = 1, keyOffset = 0 } = {}) {
  const rawKeys = KEYS();
  if (!rawKeys.length) throw new Error("designgen: GEMINI_KEYS not set");
  // keyOffset pins each parallel page to a DIFFERENT key first (page i starts on
  // key i), so 4 concurrent generations don't collide on one key's per-minute
  // rate limit. The remaining keys still follow as fallback, so a 429 or quota
  // hit on the primary key degrades gracefully instead of failing the page.
  const off = ((keyOffset % rawKeys.length) + rawKeys.length) % rawKeys.length;
  const keys = rawKeys.slice(off).concat(rawKeys.slice(0, off));
  let last;
...
// The headers a real Chrome sends. The old request announced itself as
// "Mozilla/5.0 DesignGen", which is not a browser any bot-detector has heard of,
// and a UA alone with none of its usual companions is a strong bot signal.
// Cloudflare let it through from a laptop and refused it 403 from the deployed
// tool's datacenter IP — same code, same site, different address.
...
async function checkUrl(u) {
  const probe = async (method, headers) => {
    ...
    try {
      const r = await fetch(u, { method, headers: { ...BROWSER_HEADERS, ...headers }, redirect: "follow", signal: ac.signal });
      return r.status;
...
/**
 * The reference site's HTML, by whichever route the site will actually allow.
 *
 * A plain fetch is tried first because it is cheap and it is all most sites
 * need. When the site refuses it — a bot wall answering 403, or the request
 * failing outright — we ask the browser this function's caller was about to
 * launch anyway: real Chrome, real TLS fingerprint, real headers. That is the
 * request Cloudflare is looking for, and it is the difference between "this
 * client's site cannot be used as a reference" and a build.
 *
 * Only the HTML comes from here. Screenshots, palette and image harvesting are
 * unchanged downstream.
 *
 * @returns {Promise<{html: string, finalUrl: string, via: string}>}
 */
async function fetchReferenceHtml(full, log) {
  let firstProblem = "";
  try {
    const r = await fetch(full, { redirect: "follow", headers: BROWSER_HEADERS });
    if (r.ok) {
      const html = await r.text();
      // A bot wall can answer 200 with a challenge page. Real pages have a body
      // worth reading; a challenge is a few kilobytes of script.
      if (html.length > 2000) return { html, finalUrl: r.url, via: "fetch" };
      firstProblem = `HTTP 200 but only ${html.length} bytes — looks like a challenge page`;
    } else {
      firstProblem = `HTTP ${r.status}`;
    }
  } catch (e) {
    firstProblem = e.message;
  }

  log(`direct fetch refused (${firstProblem}) — retrying through a real browser`);

  // Browserless first, local Playwright second — the same order the rest of the
  // tool uses for browsers, and for the same reason: Render's node runtime has
  // the playwright package but not a usable Chromium (render-build.sh installs
  // no browser, and the sandbox has none of Chromium's shared libraries), so on
  // the deployed tool this hosted route is the only one that runs. On a laptop
  // it is the other way round and the local browser takes over.
  const blToken = process.env.BROWSERLESS_TOKEN || process.env.BROWSERLESS_API_KEY || "";
  let blProblem = "not configured (BROWSERLESS_TOKEN)";
  if (blToken) {
    const base = String(process.env.BROWSERLESS_URL || "https://production-sfo.browserless.io").replace(/\/+$/, "");
    const ctl = new AbortController();
    const timer = setTimeout(() => ctl.abort(), 45000);
    try {
      const res = await fetch(`${base}/content?token=${encodeURIComponent(blToken)}`, {
        method: "POST", signal: ctl.signal, headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          url: full,
          gotoOptions: { waitUntil: "networkidle2", timeout: 40000 },
          waitForTimeout: 3500,   // let a Cloudflare interstitial resolve itself
          bestAttempt: true,
        }),
      });
      const body = await res.text();
      if (res.ok && body.length > 2000) {
        log(`browserless fetch worked (${Math.round(body.length / 1024)}kb)`);
        return { html: body, finalUrl: full, via: "browserless" };
      }
      blProblem = res.ok ? `only ${body.length} bytes` : `HTTP ${res.status}`;
    } catch (e) {
      blProblem = e.message;
    } finally { clearTimeout(timer); }
    log(`browserless refused (${blProblem}) — trying a local browser`);
  }
  ...
  try {
    ...
    const resp = await p.goto(full, { waitUntil: "domcontentloaded", timeout: 60000 });
    // Cloudflare's interstitial resolves itself after a moment; give it one.
    await p.waitForTimeout(3500);
    const html = await p.content();
    const finalUrl = p.url();
    const status = resp ? resp.status() : 0;
    if (!html || html.length < 2000) {
      throw new Error(`browser got HTTP ${status} and ${html ? html.length : 0} bytes`);
    }
    log(`browser fetch worked (HTTP ${status}, ${Math.round(html.length / 1024)}kb)`);
    return { html, finalUrl, via: "browser" };
  } catch (e) {
    throw new Error(`reference site could not be read — direct fetch: ${firstProblem}; browserless: ${blProblem}; local browser: ${e.message}`);
  } finally {
...
async function scrape(url, log) {
  ...
  const fetched = await fetchReferenceHtml(full, log);
  const html = fetched.html;
  // Relative URLs resolve against wherever we actually landed, which may differ
  // from `full` after redirects — and differ again between the two routes.
  const r = { url: fetched.finalUrl };
  ...
  await Promise.all(cssLinks.map(async (cu) => {
    try { const cr = await fetch(cu, { headers: BROWSER_HEADERS }); if (cr.ok) countColors((await cr.text()).slice(0, 400000)); } catch (e) { /* best effort */ }
  }));
...
Reply ONLY JSON: {"best": <1-based index of the best usable photo, or 0 if none qualify>}` }];
        ...
        for (const im of shortlist) {
          try {
            const rr = await fetch(im.src, { headers: BROWSER_HEADERS });
            if (!rr.ok) continue;
...
${s.subFacts.map((f) => `--- ${f.kind.toUpperCase()} PAGE DATA ---\n${f.headings.join(" · ")}\n${f.text.slice(0, 2000)}`).join("\n")}`;
  ...
  // Build one page (its own retry loop + post-processing). Returns {key, html}.
  const buildPage = async (pg, idx) => {
    ...
    while (attempt < 3) {
      ...
      const p2 = await gemini([{ text: bp }], {
        ...
        waves: attempt < 3 ? 2 : 1,
        keyOffset: idx,   // page i starts on key i → no cross-page rate-limit collision
      });
    ...
    return { key: pg.key || "home", html };
  };
  // Generate all 4 pages IN PARALLEL. The pages are independent (chrome is
  // unified afterward), each pinned to its own Gemini key, so total wall-clock
  // is the slowest single page instead of the sum of all four (~4x faster).
  const results = await Promise.all(PAGE_DEFS.map((pg, i) => buildPage(pg, i)));
  const pages = {};
  for (const r of results) pages[r.key] = r.html;
...
// unifyChrome, fetchReferenceHtml and BROWSER_HEADERS are exported for tests
module.exports = { generateDesigngenSite, brandTokens, withMotionLayer, SITE_JS, unifyChrome, fetchReferenceHtml, BROWSER_HEADERS };
