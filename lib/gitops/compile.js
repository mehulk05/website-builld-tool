...
//   NOT rewritten (MediaResolver::resolvePlaceholders uses str_starts_with, so
//   it cannot see a ref in the middle of a string). Images therefore reference
//   their absolute source URL — a hotlink — unless they sit in a control;
//   assets/img/* local paths are mapped back via GEN/site/assets/img-map.json.
//   GITOPS_IMAGE_CONTROLS=on lifts a section's backdrop into the container's
//   background_image control, which the pre-release image pass can then localise.
...
// ---- images into Elementor media controls -----------------------------------
// g99-control's MediaResolver resolves "media:<ref>" in exactly two shapes: a
// whole string, or an object whose `id` is one — the Elementor media control
// {url, id, size, alt, source}, whose `url` it then rewrites to the real
// attachment URL. It never looks inside markup, so an <img src> in an html
// widget can be localised on disk and still have nothing to point at.
//
// Every generated section puts its photo in a dedicated wrapper —
// <div class="c-hero__media"><img ...></div> — so a section whose photo is
// really a backdrop can hand it to the container and drop the wrapper. The
// pre-release image pass then localises it like any other control.
//
// Off by default: this removes an element the section's own CSS was written
// around, so it changes rendering and wants a real Elementor render per template
// before it becomes how every site is built.
const MEDIA_CONTROLS = String(process.env.GITOPS_IMAGE_CONTROLS || "").toLowerCase() === "on";

// The substring of `html` starting at `from` that closes the element opening
// there, brace-... tag-depth aware so a nested <div> does not end it early.
function balancedFrom(html, from) {
  const re = /<(\/?)([a-zA-Z][a-zA-Z0-9-]*)((?:"[^"]*"|'[^']*'|[^>"'])*)>/g;
  re.lastIndex = from;
  let depth = 0, m;
...
// A section with exactly ONE "__media" wrapper holding exactly ONE <img> is a
// backdrop. Anything else — a five-card grid, a gallery — has images that are
// content in their own right, and a container has only one background, so those
// are left in markup for now.
function liftSectionBackground(chunk) {
  const wrappers = [...chunk.matchAll(/<div\b[^>]*\bclass=["'][^"']*__media[^"']*["'][^>]*>/gi)];
  if (wrappers.length !== 1) return null;
  const block = balancedFrom(chunk, wrappers[0].index);
  if (!block) return null;
  const tags = block.match(/<img\b[^>]*>/gi) || [];
  if (tags.length !== 1) return null;
  const src = (tags[0].match(/\ssrc\s*=\s*["']([^"']+)["']/i) || [])[1] || "";
  if (!/^https?:\/\//i.test(src)) return null;
  const alt = (tags[0].match(/\salt\s*=\s*["']([^"']*)["']/i) || [])[1] || "";
  return {
    html: chunk.slice(0, wrappers[0].index) + chunk.slice(wrappers[0].index + block.length),
    // `id` is empty here on purpose: compile only puts the photo somewhere a ref
    // CAN live. The pre-release image pass downloads it, writes
    // resources/media/<ref>.{webp,json} and fills the id in.
    settings: {
      background_background: "classic",
      background_image: { url: src, id: "", size: "", alt, source: "library" },
...
function elementorDoc(slug, css, body, cssInline = false) {
  ...
  return {
    ...
    elements: parts.map((chunk, i) => {
      const lifted = MEDIA_CONTROLS ? liftSectionBackground(chunk) : null;
      const html = lifted ? lifted.html : chunk;
      return {
...
function compileGitops(siteDir, biz, existingGitId = () => null, opts = {}) {
  ...
  // Site-wide JavaScript (e.g. the designgen premium motion engine). The plugin
  // kses-strips <script> from html widgets, so page JS is shipped instead as a
  // single global Elementor Pro "Custom Code" snippet (body_end, all pages) —
  // the same channel mcptest2 already uses. Only emitted when the engine wrote
  // assets/site.js, so other engines (webgen) are unaffected.
  const siteJsFile = path.join(siteDir, "assets", "site.js");
  if (fs.existsSync(siteJsFile)) {
    // Raw JS in both snippets, NOT wrapped in <script>: the reconcile writes
    // post_content without unfiltered_html, so kses would strip a stored tag.
    // The carrier wraps at output time (WPCode natively; elementor_snippet via
    // the g99-control wp_footer hook).
    const js = fs.readFileSync(siteJsFile, "utf8");
    // PRIMARY channel: WPCode (free, Elementor-Pro-independent, admin-audited).
    // Renders on any site with the WPCode plugin once g99-control's
    // CptSyncConfig::POST_TYPES includes "wpcode" (canonical rollout).
    files.set("resources/cpt/wpcode/g99-site-js/cpt.json", JSON.stringify({
      schema_version: 1,
      git_id: "cpt-wpcode-g99-site-js",
      type: "wpcode",
      slug: "g99-site-js",
      title: "G99 generated site JavaScript",
      status: "publish",
      content: js,
      meta: { _wpcode_code_type: "js", _wpcode_location: "site_wide_footer", _wpcode_auto_insert: "1", _wpcode_priority: "10" },
      media: [],
    }, null, 4));
    // LEGACY channel: Elementor Pro Custom Code — kept so sites already running
    // motion through it (self-healed g99-control wp_footer, e.g. mcptest2) keep
    // working. Importers skip whichever type their site doesn't support.
    files.set("resources/cpt/elementor_snippet/g99-site-js/cpt.json", JSON.stringify({
      schema_version: 1,
      git_id: "cpt-elementor-snippet-g99-site-js",
      type: "elementor_snippet",
      slug: "g99-site-js",
      title: "G99 generated site JavaScript",
      status: "publish",
      content: js,
      meta: { _elementor_location: "body_end", _elementor_conditions: ["include/general"] },
      media: [],
    }, null, 4));
  }
  return { files, pages };
...
// elementorDoc and liftSectionBackground are exported so the image-control
// conversion can be exercised against a real page without running a build.
module.exports = { compileGitops, elementorDoc, liftSectionBackground, topLevelChunks };
