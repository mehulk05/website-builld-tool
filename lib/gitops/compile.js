...
const { chunkToContainer, bridgeCss, countWidgets } = require("./widgets.js");
...
// The beta review bar (reviewBanner() in server.js) belongs to the local review
// bundle only: it links to branding.html / seo.html, which exist in the bundle
// and 404 on WordPress. It reached a live client site once — strip it here, where
// every page funnels through, rather than in any one caller.
function stripReviewBar(html) {
  return html.replace(/<div\b[^>]*\bdata-g99-review\b[^>]*>[\s\S]*?<\/div>/gi, "");
}
...
//
// `sitePhotos` MUST be the images belonging to the site being compiled. This used
// to read Object.values(imgMap) instead — but img-map.json is a local-path→source
// TRANSLATION map that localizeImages merges into rather than replaces, so it
// accumulated every image of every client built on the deployment. Using it as the
// spare pool substituted a PREVIOUS client's photograph into this client's page
// whenever a page repeated an image. Reproduced by test-webgen.js.
function dedupeImages(html, sitePhotos) {
  const DECOR = /logo|icon|favicon|sprite|\.svg(\?|$)/i;
  const used = new Set([...html.matchAll(/<img\b[^>]*\bsrc=["']([^"']+)["']/gi)].map((m) => m[1]));
  const spare = (Array.isArray(sitePhotos) ? sitePhotos : []).filter((u) => !used.has(u) && !DECOR.test(u));
  const seen = new Set();
...
function elementorDoc(slug, css, body, cssInline = false, native = true) {
  ...
  // Native widgets: each chunk becomes a container tree of real Heading /
  // Button / Image / Text Editor widgets so the client edits fields, not code.
  // A chunk the converter does not recognise falls through to the html widget
  // below, so this can never drop a section.
  const pageCss = native ? bridgeCss(css) : css;
  return {
    schema_version: 1,
    elementor_version: "3",
    ...
    document_settings: { custom_css: pageCss },
    elements: parts.map((chunk, i) => {
      if (native) {
        const tree = chunkToContainer(chunk, slug, i, crypto);
        if (tree) return tree;
      }
      return legacyChunk(slug, chunk, i);
    }),
  };
}

// The original one-container-one-html-widget shape. Still the fallback for any
// chunk the widget converter cannot name, and for native:false.
function legacyChunk(slug, chunk, i) {
  return {
...
function compileGitops(siteDir, biz, existingGitId = () => null, opts = {}) {
  ...
  // First pass: the images THIS site actually uses, in page order. This is the only
  // legitimate spare pool for de-duplication — img-map.json cannot be used for it,
  // because it accumulates across builds and would hand one client another client's
  // photographs.
  const sitePhotos = [];
  for (const p of PAGES) {
    const src = path.join(siteDir, p.file);
    if (!fs.existsSync(src)) continue;
    const html = unlocalizeImages(fs.readFileSync(src, "utf8"), imgMap);
    for (const m of html.matchAll(/<img\b[^>]*\bsrc=["']([^"']+)["']/gi)) {
      if (!sitePhotos.includes(m[1])) sitePhotos.push(m[1]);
    }
  }
  ...
  for (const p of PAGES) {
    ...
    html = rewriteLinks(stripReviewBar(dedupeImages(unlocalizeImages(html, imgMap), sitePhotos)));
